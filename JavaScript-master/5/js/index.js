...
$(function () {
    ...
    $menu.singleTap(function () {
        if (!menuFlag) {//->当前是隐藏的我让其显示 //这个是使用的animta动画anmiantion动画
            //还不如直接使用 设置，因为显示，隐藏，透明度不能直接设置，所以先让其显示，在让透明度发生变化，这样就可以实现了
...
$(function () {
    ...
    //->每一次加载页面之前我们先判断之前是否已经存储过数据,并且存储的时间是否还在有效期内,如果还在有效期内,我们不需要重新的请求新的数据了  localstorage只能存字符串，
    var matchStorage = localStorage.getItem("matchStorage");
    if (matchStorage) {
        matchStorage = JSON.parse(matchStorage);
        var time = matchStorage["time"];//最多就是防止1分钟内连续发送请求，页面刷新就会像服务器发送一次请求；刷新一次就需要看一次本地localstorage存在吗，1分钟只能不能再发送请求；
        //1分钟之内刷新页面 还是需要重i新绑定的 但是是从本地拿数据 不经过 ajax请求了
    ...
    function callback(jsonData) {
        ...
        if (jsonData && jsonData[1]) {
            ...
            str += '</div>';
            //下面的这个就是 leftSupport就是服务器返回的数据中存在的；
        ...
        //->把本次请求回来的数据存储到本地
        var matchStorage = {
            time: new Date().getTime(),//在1分中之内我还是不断的记录我的电脑时间，只要你不挺的刷新，我本地的缓存还是不变的，变得只有时间；
            data: jsonData//这个数据必须全部保存在本地； 这样在本地不断刷新的时候，我们可以正常的将数据绑定在页面中；
        };
        console.log(jsonData);//我们获取到数据之后我们自己存到本地；
        ...
        //未点击之前，还没有进行本地缓存，所以第一次获取的是undefined
        if (storageInfo) {
    ...
    function supportFn() {
        ...
        //第一次点击的时候 还没有 这个属性，设置自定属性，同时放在本地同时还要告诉服务器你点击的是那个一个  通过ajax请求；
        $sup.attr("isTouch", "ok");
        ...
        //->向后台的服务器发送请求告诉它我点击的是哪一个*********** type=1后台就是通过这个接受
